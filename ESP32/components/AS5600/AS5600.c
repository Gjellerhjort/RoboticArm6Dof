...
// Define the AS500 to use the i2c nummer 0 
...
uint16_t AS5600_read()
{
    ...
    // sends the command to read angle of the AS5600 sensor
    ...
    else
    {
        ...
        uint8_t angle_data[2]; // make array of two bytes
        cmd = i2c_cmd_link_create();
        i2c_master_start(cmd);
        i2c_master_write_byte(cmd, (AS5600_SENSOR_ADDR << 1) | I2C_MASTER_READ, true);
        // reads the bytes from AS5600
        ...
        i2c_cmd_link_delete(cmd);
        // Compines the two bytes to a 16 bit integere
